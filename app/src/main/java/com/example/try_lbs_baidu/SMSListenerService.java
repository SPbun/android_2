...
public class SMSListenerService extends Service
{
  ...
  //短信监听关键字
  private static String SMSKeyword ="定位";
  private String PhoneNumber;
  private String BroadCasstMessage;
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      if (intent.getAction().equals(SMS_Coming_Broadcast)) {
        ...
        if ((messages = GetMessageInfo(intent)) != null) {
          /* 向系统广播自定义信息 */
         BroadCasstMessage= buildSMSBroadcastString(messages);
          if(isCommandSMS(SMSKeyword,BroadCasstMessage)) {
            sendBroadcastInformatin(StartActivityBroadcast, "STR_PARAM01", BroadCasstMessage);
          }
        }
      }
    }
    /* 判断接收的短信是否为有关键字的短信 */
    public boolean isCommandSMS(String strPat, String strSMS)
    {
